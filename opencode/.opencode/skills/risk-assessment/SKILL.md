...
description: Identify and assess technical, dependency, and scope risks
license: MIT
---
...
3. **DO NOT** add preamble text like "Here is the risk assessment..." or "Given the input..."
...
6. **USE THE USER'S INPUT** - extract risks, constraints from THEIR message
7. **STAY ON TOPIC** - the assessment must be about the user's project (healthcare, appointments, EHR, etc)
8. **IMMEDIATELY** output the risk assessment markdown (not code or explanation)
9. Your **FIRST CHARACTER** must be `#` - start with `# Risk Assessment:` directly

**IMPORTANT**: Output ONLY the raw markdown document. No preamble. No code blocks. First character is `#`.

## Output Format
Output MUST be valid markdown starting with `# Risk Assessment:`.

## Example Output

```markdown
# Risk Assessment: Patient Appointment Portal
